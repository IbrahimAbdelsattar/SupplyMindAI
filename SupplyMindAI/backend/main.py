...
def bg_retrain_task():
    try:
        ...
        ML_MODEL = init_ml_model(STORE)
        print("ML Model re-training completed successfully and reloaded!")
    except Exception as e:
        print(f"Error during ML Model re-training: {e}")


@app.post("/api/v1/mlops/retrain")
def retrain_model_endpoint(background_tasks: BackgroundTasks, user: User = Depends(_require_roles("admin"))):
    background_tasks.add_task(bg_retrain_task)
    return {"status": "accepted", "message": "ML Model re-training initiated in the background."}
