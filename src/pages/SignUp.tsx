    ...
    <div className="min-h-screen flex items-center justify-center bg-background py-12">
      ...
      >
        <div className="text-center mb-8">
          ...
          <h1 className="text-h1 mb-2 text-foreground">{t('signUp:title') ?? 'Create account'}</h1>
          <p className="text-body text-muted-foreground">{t('signUp:subtitle') ?? 'Join to continue'}</p>
        </div>

        <Card className="border border-border">
          <CardHeader className="pb-4">
            <CardTitle className="text-h2">{t('signUp:cardTitle') ?? 'Sign up'}</CardTitle>
            <CardDescription>{t('signUp:cardDescription') ?? 'Use your email to create an account.'}</CardDescription>
          </CardHeader>
